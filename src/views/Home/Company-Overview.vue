    ...
    <div class="content-container--large text--light content-container" id="what-is-matryx">
        ...
        <div class="overview__column-1 mobile--order-1">
          <h3 class="text-color--matryx-black subheader">
            <span v-if=" language === 'ru' ">Основные сведения о Matryx</span>
          	<span v-else-if=" language === 'ch' ">Matryx 背景</span>
          	<span v-else-if=" language === 'ge' ">Hintergrund von Matryx</span>
          	<span v-else-if=" language === 'ja' ">Matryxの背景</span>
          	<span v-else-if=" language === 'ko' ">Matryx가 만들어진 배경</span>
          	<span v-else>Matryx Background</span>
          </h3>
          <p class="text-color--matryx-dark-grey">
            <span v-if=" language === 'ru' ">Matryx является продуктом компании <a target="_blank" href="http://nanome.ai/">Nanome</a>. Nanome стремится к демократизации науки, техники и научных исследований с использованием технологий виртуальной реальности и блокчейна. Nanome — создатель Calcflow, лидирующего в отрасли набора математических инструментов ВР, и nano-one — интерфейса ВР для наномасштабного проектирования и моделирования.</span>
          	<span v-else-if=" language === 'ch' ">Matryx 是 <a target="_blank" href="http://nanome.ai/">Nanome</a> 的产品。Nanome 利用虚拟现实和区块链技术，寻求民主化科学、设计和研究。Nanome 创造了行业领先的虚拟现实数学工具包 Calcflow，还创造了纳米级设计和仿真用虚拟现实界面 nano-one。</span>
          	<span v-else-if=" language === 'ge' ">Matryx ist ein Produkt von <a target="_blank" href="http://nanome.ai/">Nanome</a>. Nanome setzt sich für die Demokratisierung von Wissenschaft, Technik und Forschung mit Hilfe von Virtual Reality und Blockchain-Technologie ein. Nanome ist Hersteller von Calcflow, dem branchenweit führenden VR-Mathematik-Toolkit, sowie von nano-one, einem VR-Interface für Design und Simulation im Nanomaßstab.</span>
          	<span v-else-if=" language === 'ja' ">Matryxは<a target="_blank" href="http://nanome.ai/">Nanome</a> の製品です。Nanomeは、バーチャルリアリティとブロックチェーンテクノロジーを使って科学、工学、研究を民主化しようとしています。Nanomeは、業界をリードするVR数学ツールキットであるCalcflowと、ナノスケール設計とシミュレーションのためのVRインターフェースであるnano-oneの製作者です。</span>
          	<span v-else-if=" language === 'ko' ">Matryx는 <a target="_blank" href="http://nanome.ai/">Nanome</a>의 제품입니다. Nanome은 가상현실 및 블록체인 기술을 이용하여 모든 이용자들이 참여하는 과학, 공학, 연구를 실현시킬 수 있는 방법을 찾기 위해 노력하고 있습니다. Nanome은 업계를 선도하는 VR 수학 툴킷 Calcflow와 나노 수준의 정밀도를 가진 디자인 및 시뮬레이션을 위한 VR 인터페이스인 nano-one의 제작자이기도 합니다.</span>
          	<span v-else>Matryx is a product of <a target="_blank" href="http://nanome.ai/">Nanome</a>. Nanome seeks to democratize science, engineering, and research using Virtual Reality and blockchain technology. Nanome is the creator of Calcflow, the industry-leading VR mathematics toolkit, and nano-one, a VR interface for nanoscale design and simulation.</span>
          </p>
        </div>
      ...
      <div class="overview">
        ...
        <div class="overview__column-1">
          ...
          <p class="text-color--matryx-dark-grey">
            <span v-if=" language === 'ru' ">Решение самых сложных проблем в мире в самодостаточных подразделениях приводит к дублированию усилий и избыточности. Создания глобального сообщества, которое поощряет и стимулирует обмен идеями, значительно сокращает время создания инновации и позволяет решать проблемы с такой высокой скоростью, которая была бы невозможна в другой ситуации.<br/> <br/>Усилия в области исследований и инженерных разработок являются разрозненными. Время и деньги тратятся впустую, когда различные команды работают над одной и той же проблемой без координации между собой. Признание и гранты получают последние люди, которые работали над проблемой, а десятки важнейших соавторов остаются без внимания. Таким образом, нет никаких стимулов для сотрудничества с другими группами и никакого хранилища сведений о решенных проблемах.</span>
          	<span v-else-if=" language === 'ch' ">在筒仓内解决世上最严重的问题导致成倍的工作和冗余。通过提供激励创意交换的全球社区，创新时间被极大缩减，同时问题以在其他场合不可能实现的速度被解决。<br/> <br/>研究和设计工作碎片化。当不同的团队互不合作地应对相同的问题，时间和金钱都浪费了。赞誉和津贴归到应对某问题的最后一人，而忽视了几十位关键贡献者。同样地，没有与其他团队合作的动机，没有已解决问题的知识库。</span>
          	<span v-else-if=" language === 'ge' ">Die schwierigsten Probleme der Welt isoliert zu lösen führt zur Verdopplung des Aufwandes und zu Redundanz. Durch die Bereitstellung einer globalen Community, die den Austausch von Ideen fördert und anregt, wird die Innovationszeit deutlich reduziert und Probleme werden mit einer ansonsten unerreichbaren Geschwindigkeit gelöst.<br/><br/>Bemühungen in Forschung und Technik sind fragmentiert. Es werden Zeit und Geld verschwendet, wenn verschiedene Teams ohne Koordination an demselben Problem arbeiten. Anerkennung und Fördergelder gehen an diejenigen, die als Letzte an einem Problem gearbeitet haben, während Dutzende entscheidender Mitwirkender unbeachtet bleiben. Es besteht somit kein Anreiz zur Zusammenarbeit mit anderen Teams und es existiert kein Archiv bereits gelöster Probleme.</span>
          	<span v-else-if=" language === 'ja' ">世界でも最も困難な問題を個々に解決しようとすれば、労力の重複と冗長性につながります。アイデアの交換を奨励するグローバルなコミュニティを提供することにより、イノベーションにかかる時間が大幅に短縮され、他の方法では不可能な速度で問題が解決されます。<br/> <br/>研究と工学の努力は分断されており、異なるチームが同じ問題を調整せずに作業する場合、時間と資金が無駄になります。評価と助成金は、問題について最後に取り組んだ人々に送られ、何十人という重要な貢献者達を無視してしまいます。したがって、他のチームと協力するインセンティブはなく、解決された問題の保存場所もありません。</span>
          	<span v-else-if=" language === 'ko' ">세계에서 가장 높은 난이도를 가진 문제들을 서로 고립된 환경에서 해결하는 것은 노력의 중복과 낭비를 야기하게 됩니다. 아이디어의 교환을 장려하고 제도적으로 그것에 대해 보상을 해주는 전세계적 커뮤니티를 마련함으로써 혁신을 위해 소비되는 시간이 엄청나게 단축되게 됨은 물론 많은 문제들이 이전에는 불가능하다고 여겨졌던 속도로 해결됩니다.<br/> <br/>대체로 연구 및 공학 분야에서 투자되는 노력은 파편화되어 있기 마련입니다. 복수의 팀들이 서로 간의 조율없이 동일한 문제를 해결하기 위해 노력할 때 시간과 돈이 낭비됩니다. 인지도와 보상은 문제의 해결을 위해 가장 나중에 노력한 사람들에게 돌아가며 수십명의 중요한 기여자들의 노력은 무시됩니다. 당연히 다른 팀들과의 협동에 대해 주어지는 보상이 없음은 물론 해결된 문제들을 위한 제대로된 보관소도 갖추어져 있지 않습니다.</span>
          	<span v-else>Solving the world’s hardest problems in silos leads to duplication of effort and redundancy. By providing a global community that encourages and incentivizes the exchange of ideas, time to innovation is greatly reduced while problems are solved at a rate otherwise impossible.<br/> <br/>Research and engineering efforts are fragmented. Time and money are wasted when different teams work on the same problem without coordination. Recognition and grants go to the last people to work on a problem, ignoring dozens of critical contributors. As such there’s no incentive to cooperate with other teams, and no repository of solved problems.</span>
          </p>
        </div>
      </div>
      <div class="overview">
        <div class="overview__column-1 mobile--order-1">
          <h3 class="text-color--matryx-black subheader">
            <span v-if=" language === 'ru' ">Решение Matryx</span>
          	<span v-else-if=" language === 'ch' ">Matryx 解决方案</span>
          ...
          <p class="text-color--matryx-dark-grey">
            <span v-if=" language === 'ru' ">
          	...
          	<span v-else-if=" language === 'ch' ">
          	  Matryx 的基于区块链奖励金制度促进了协作解决问题。创造者因贡献 MTX 代币而获得回报。任何人均可发布奖励金到 Matryx。各参与者在锦标赛中竞争以求解决这些问题。
              <br><br>每一轮胜出的解决方案被公开分享，以便新的贡献者可以融入新的迭代，使用户更快、更有效地实现目标。问题一旦解决，奖励和赞誉会被赋予所有胜出的贡献者，而不仅仅是最后一位贡献者，以体现公平和透明。
          	</span>
              ...
              <br><br>各ラウンドで勝利した解決策は公に共有され、新しい貢献者が新しいバージョンにリミックスでき、ユーザーはより迅速かつ効果的に目標に到達できます。問題が解決されると、公平性と透明性のために、最後の投稿者だけでなく、勝利した貢献者全員に報奨と評価が与えられます。
          	</span>
              ...
              <br><br>각 라운드에서 우승하는 해결책은 모든 사람들에게 공유되며 이와 더불어 새 기여자들이 새로운 버전으로 재변형을 할 수 있기 때문에 이용자들이 목표에 더 빠르고 더 효율적으로 도달할 수 있습니다. 문제가 해결되면 보상 및 인지도는 공정성과 투명성을 위해 가장 나중의 기여자가 아닌 우승한 모든 기여자들에게 주어집니다.
          	</span>
          	<span v-else>
